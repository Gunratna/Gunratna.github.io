...
export const projects: Project[] = [
  {
    ...
    tagline:
      "Fine-tuned Gemma-3 VLM reading structured fields off scanned redemption forms fully on-prem — replacing a 5-call-per-form Gemini pipeline with a single self-hosted model on one GPU.",
    year: 2026,
    type: "VLM",
    featured: true,
    outcomes: [
      { label: "Gemma-3 LoRA (macro-recall)", value: "91%" },
      { label: "vs. Florence LoRA ceiling", value: "60%" },
      { label: "Gemini calls replaced", value: "5→1 per form" },
    ...
    details: [
      ...
      "Fine-tuned a Florence-2 Reader via LoRA (Locator and Reader as separate adapters, never stacked) on the annotated crops; iterative sweeps lifted field-reading macro-recall off a 10.7% base but plateaued around a 60% ceiling on a held-out test split.",
      "Pivoted to fine-tuning Gemma-3 4B end-to-end (LoRA, rank 8) as a boxless, single-call replacement for the legacy 5-call-per-form Gemini pipeline — evaluated alongside Phi-3.5-Vision and SmolVLM2, both dropped for weak accuracy. After root-causing a 'prompt parroting' failure mode (the model copying illustrative example values instead of reading the form) and applying Pan-and-Scan tiling for small handwriting, the fine-tuned Gemma-3 reached 91% macro-recall across all 9 fields at ~250 forms/hr on a single on-prem GPU — selected as the production model.",
    ],
    arch: {
      caption:
        "Two LoRA fine-tuned candidates — a staged Florence-2 pipeline and an end-to-end Gemma-3 VLM — are benchmarked on a frozen test split; Gemma-3 wins and replaces the legacy Gemini pipeline.",
      nodes: [
        ...
        { id: "vlm", label: "Gemma-3 VLM", sub: "end-to-end · LoRA r8", kind: "model" },
